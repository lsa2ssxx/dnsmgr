...
CREATE TABLE `dnsmgr_dmtask` (
  ...
  `proxy_id` tinyint(2) NOT NULL DEFAULT 0 COMMENT '0=不使用, 1=代理1, 2=代理2',
  `hook_enable` tinyint(1) NOT NULL DEFAULT 0 COMMENT '切换备用解析成功后执行Hook命令',
  `hook_cmd` varchar(512) DEFAULT NULL COMMENT 'Hook命令（单行，支持{ip}占位符）',
